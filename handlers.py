...
async def handle_business_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if text == "/start":
            ...
            # Отправляем приветствие с кнопками меню
            reply_markup = ReplyKeyboardMarkup(MAIN_MENU, resize_keyboard=True)
            await context.bot.send_message(
        ...
        # Обработка кнопок меню для бизнес-чата
        if text in ["📋 Услуги", "💰 Цены", "📞 Консультация", "❓ Помощь"]:
            # Создаем временный update для handle_menu_button
            # (функция ожидает обычное сообщение, адаптируем)
            menu_responses = {
                "📋 Услуги": (
                    "🎯 НАШИ УСЛУГИ:\n\n"
            ...
            response_text = menu_responses.get(text, "Выберите пункт меню")
            await context.bot.send_message(
                chat_id=message.chat.id,
                text=response_text,
                business_connection_id=message.business_connection_id
